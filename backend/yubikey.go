...
func NewYubikeyKey(yubikeyReader *config.YubikeyReader, hier hierarchy.Hierarchy, keyConfig *config.KeyConfig) (*Yubikey, error) {
	...
	if cert != nil && !yubikeyReader.Overwrite {
		...
		case *ecdsa.PublicKey:
			// EC Public Key
			keyAlgName = yubiPub.Params().Name
			logging.Println(fmt.Sprintf("Using existing EC %s Key MD5: %x in Yubikey PIV %s Slot", keyAlgName, md5sum(cert.PublicKey), slotName))
			keyAlg = x509.ECDSA
		...
		case "P-256":
			pivAlg = piv.AlgorithmEC256
		case "P-384":
			pivAlg = piv.AlgorithmEC384
		default:
			if !yubikeyReader.Overwrite {
				return nil, fmt.Errorf("yubikey: unsupported existing yubikey key algorithm: %s", keyAlg)
			}
		}
