...
func RunCreateKeys(state *config.State) error {
	...
	if !sbctl.CheckIfKeysInitialized(state.Fs, state.Config.Keydir) {
		var hier *backend.KeyHierarchy
		if PKKeytype == "" && KEKKeytype == "" && DbKeytype == "" {
			hier, err = backend.CreateKeys(state)
		} else {
			var h backend.KeyHierarchy
			if PKKeytype != "" && !sbctl.CheckIfKeyFileExists(state.Fs, state.Config.Keydir, hierarchy.PK.String()) {
				h.PK, err = backend.CreateKey(state, state.Config.Keys.PK, hierarchy.PK)
			} else {
				h.PK, err = backend.GetKeyBackend(state, hierarchy.PK)
			}
			if err == nil {
				if KEKKeytype != "" && !sbctl.CheckIfKeyFileExists(state.Fs, state.Config.Keydir, hierarchy.KEK.String()) {
					h.KEK, err = backend.CreateKey(state, state.Config.Keys.KEK, hierarchy.KEK)
				} else {
					h.KEK, err = backend.GetKeyBackend(state, hierarchy.KEK)
				}
			}
			if err == nil {
				if DbKeytype != "" && !sbctl.CheckIfKeyFileExists(state.Fs, state.Config.Keydir, hierarchy.Db.String()) {
					h.Db, err = backend.CreateKey(state, state.Config.Keys.Db, hierarchy.Db)
				} else {
					h.Db, err = backend.GetKeyBackend(state, hierarchy.Db)
				}
			}
			if err == nil {
				hier = &h
